                    ...
                    for permute in data_params['permute']:
                        ...
                        if synthetic_seed > 0 and seed > 0:
                            ...
                            if not vary_set_length:
                                self.assertTrue(
                                    sample1[0][0, 0] in sample2[0][0, 0]
                                )
                            else:
                                for k in range(len(sample1) - 1):
                                    self.assertFalse(
                                        torch.equal(sample1[k], sample2[k])
                                    )
                            ...
                            if permute:
                                ...
                                self.assertTrue(
                                    torch.equal(sample1[3], sample2[3])
                                )
                            else:
                                self.assertEqual(sample1[4], sample2[4])
                                for k in range(len(sample1) - 1):
                                    self.assertFalse(
                                        torch.equal(sample1[k], sample2[k])
                                    )
                ...
                for seed in data_params['seed']:
                    for permute in data_params['permute']:
                        ...
                        collator = CollatorSetMatching(
                            data_params['data_dim'], data_params['set_length'],
                        ...
                        for batch_index, batch in enumerate(data_loader):
                            ...
                            if eval(data_params['batch_first']):
                                self.assertEqual(
                                    set1_batch.shape, (
                                        25, data_params['set_length'],
                                        data_params['data_dim']
                                    )
                                )
                                self.assertTrue(
                                    torch.unique(set1_batch, dim=0).size(0) ==
                                    25
                                )
                                self.assertTrue(
                                    torch.unique(set2_batch, dim=0).size(0) ==
                                    25
                                )
                            else:
                                self.assertEqual(
                                    set1_batch.shape, (
                                        data_params['set_length'], 25,
                                        data_params['data_dim']
                                    )
                                )
                                self.assertTrue(
                                    torch.unique(set1_batch, dim=0).size(1) ==
                                    25
                                )
                                self.assertTrue(
                                    torch.unique(set2_batch, dim=0).size(1) ==
                                    25
                                )
                            self.assertEqual(
                                set1_batch.shape, set2_batch.shape
                            )
                            ...
                            if permute:
                                ...
                                ordered_set1 = set1_batch[
                                    torch.arange(0, idx21_batch.
                                                 size(0)).unsqueeze(1).
                                    repeat((1,
                                            idx21_batch.size(1))), idx21_batch]

                                ordered_set2 = set2_batch[
                                    torch.arange(0, idx12_batch.
                                                 size(0)).unsqueeze(1).
                                    repeat((1,
                                            idx12_batch.size(1))), idx12_batch]
